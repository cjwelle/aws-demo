def authorize_key_for_ubuntu(config, *key_paths)
  [*key_paths, nil].each do |key_path|
    ...
    if File.exists?(full_key_path)
      config.vm.provision 'file',
        ...
        destination: '/home/vagrant/ubuntu_pubkey'

      config.vm.provision 'shell',
        privileged: true,
        run: 'once',
        inline:
          "echo \"Creating /ubuntu/.ssh/authorized_keys with #{key_path}\" && " +
          'rm -f /ubuntu/.ssh/authorized_keys && ' +
          'mv /home/vagrant/ubuntu_pubkey /ubuntu/.ssh/authorized_keys && ' +
          'chown ubuntu:ubuntu /ubuntu/.ssh/authorized_keys && ' +
          'chmod 600 /ubuntu/.ssh/authorized_keys && ' +
          'rm -f /home/vagrant/ubuntu_pubkey && ' +
          'echo "Done!"'
      break
    end
  end
